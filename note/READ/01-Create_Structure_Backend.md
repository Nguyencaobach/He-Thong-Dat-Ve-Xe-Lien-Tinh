...
**Phân biệt Giao tiếp Đồng bộ (Synchronous) và Bất đồng bộ (Asynchronous):**
- **Đồng bộ (Synchronous) = `gRPC` và `GraphQL`:** 
  Giống như *"Gọi điện thoại - Phải có người nhấc máy trả lời ngay lập tức"*. Ví dụ: Khi Gateway muốn tìm chuyến xe, nó bắt buộc phải gọi gRPC sang `trip-service` và đứng chờ lấy kết quả về để hiển thị lên web ngay. Nếu `trip-service` sập, chức năng này báo lỗi lập tức.
- **Bất đồng bộ Nghiệp vụ lõi (Asynchronous Core) = `RabbitMQ`:** 
  Giống như *"Gửi bưu kiện - Gửi xong đi làm việc khác, người kia sẽ mở ra xử lý sau"*. Dùng cho các nghiệp vụ sinh tử (không được mất dữ liệu) nhưng tốn thời gian (in vé, gửi email). Khách thanh toán xong là hệ thống báo thành công ngay lập tức mà không cần chờ. Đồng thời quăng tin nhắn lên RabbitMQ, tiến trình nền (`worker`) sẽ lôi ra gửi email sau. Dù module gửi email đang sập, tin nhắn vẫn an toàn trong hàng đợi chờ xử lý bù.
- **Bất đồng bộ Thống kê (Asynchronous Analytics) = `Kafka`:** 
  Giống như *"Đài quan sát khí tượng"*. Ghi nhận hàng chục ngàn sự kiện mỗi giây (mọi cú click chuột, lượt search chuyến). Luồng dữ liệu khổng lồ này được tách riêng ra Kafka để không làm nghẽn đường ống RabbitMQ (nơi xử lý tiền bạc, vé xe). Rớt mạng mất vài log thống kê thì hệ thống chính vẫn chạy mượt mà, khách vẫn mua được vé.

Dưới đây là ý nghĩa và ví dụ thực tế chi tiết hơn cho từng công nghệ được chọn trong dự án:
