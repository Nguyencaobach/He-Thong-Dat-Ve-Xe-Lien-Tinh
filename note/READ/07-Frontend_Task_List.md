...
- [ ] Khởi tạo thư mục gốc: Tạo file `.env.local` cấu hình `BACKEND_GRAPHQL_URL`, `APP_URL`, `AUTH_COOKIE_NAME`.
- [ ] **Lõi GraphQL Client (`src/lib/graphql/`)**:
  - [ ] `client.ts`: Viết Custom Fetch Client gọi tới Gateway (áp dụng Next.js cache/revalidate).
  - [ ] `documents.ts`: Định nghĩa nguyên văn các chuỗi Query/Mutation.
  - [ ] `types.ts`: Khai báo Type an toàn (TypeScript).
  - [ ] `errors.ts`: Xử lý lỗi trả về chuyên biệt cho GraphQL.
- [ ] **Lớp API Trừu tượng (`src/lib/api/`)**: Tạo các file `trips.ts`, `bookings.ts`, `admin.ts` chuyên gọi thông qua GraphQL Client.
- [ ] **Quản lý Xác thực (`src/lib/auth/`)**: Tạo `session.ts` để đọc/ghi HTTP-only Cookie và truy xuất thông tin User/Token.
- [ ] **Bảo vệ Định tuyến (Route Protection)**: Tạo file `src/middleware.ts` kiểm tra Cookie, tự động chặn và redirect về `/login` nếu truy cập `/admin` hoặc `/customer` trái phép.
- [ ] Dựng bộ Layout Dùng chung trong `src/components/layout/`: `site-header.tsx`, `site-footer.tsx`, `admin-sidebar.tsx`.

## Giai đoạn 2: Module 1 - Xác thực & Trang công khai (Public Routes)
- [ ] Thiết lập thư mục Route: `src/app/(public)` và `src/app/(auth)`.
- [ ] **Trang Xác thực (`(auth)/login`, `(auth)/register`)**:
  - [ ] Dựng UI Form với package `zod` validation.
  - [ ] **Server Actions (`src/actions/auth.ts`)**: Viết logic `loginAction`, `registerAction`, thực hiện gán HTTP-only Cookie khi đăng nhập thành công.
- [ ] **Trang Chủ (`(public)/page.tsx`)**: Giao diện Hero Banner, hiển thị danh sách Tuyến xe phổ biến.
- [ ] **Tìm kiếm Chuyến xe (`(public)/trips`)**:
  - [ ] Tạo UI Component `search-form.tsx` (Điểm đi/đến, ngày đi) có tính năng Autocomplete.
  - [ ] Fetch dữ liệu chuyến từ Server Component, hiển thị bằng card `trip-card.tsx` (trong thư mục `src/components/trip/`).

## Giai đoạn 3: Module 2 - Sơ đồ ghế & Realtime (Cốt lõi)
- [ ] Trang chi tiết chuyến (`(public)/trips/[id]/page.tsx`): Hiển thị thông tin tổng quan, điểm đón/trả.
- [ ] **Sơ đồ ghế (`src/components/trip/seat-map.tsx`)**:
...
- [ ] **Server Actions (`src/actions/seat.ts`)**: Viết logic gọi Mutation `holdSeats` (thông qua hàm Action từ Client) khi người dùng chọn ghế.
- [ ] Giao diện **Đồng hồ đếm ngược**: Component Timer 5 phút trên UI, hết giờ tự reset trạng thái ghế đã chọn.
- [ ] **Cập nhật Realtime**: Tích hợp cơ chế để Frontend nhận sự kiện ghế bị người khác chọn (qua Subscriptions hoặc Long-polling tối ưu) và đổi màu ngay lập tức trên màn hình.
